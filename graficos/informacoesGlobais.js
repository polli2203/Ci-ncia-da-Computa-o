...
async function vizualizarInformacoesGlobais() {
    ...
    const praticam_regularmente_esportes = (dados.praticam_regularmente_esportes/ 1e9)
    const pessoasNoMundo = (dados.total_pessoas_mundo / 1e9)
    const horas = parseInt(dados.tempo_medio)
    const minutos = Math.round((dados.tempo_medio_semana_praticando_esporte - horas) * 100)
    const porcentagemConectada = ((praticam_regularmente_esportes / pessoasNoMundo ) * 100).toFixed(2)
